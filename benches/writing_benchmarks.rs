...
fn benchmark_writing_integers(c: &mut Criterion) {
    ...
    group.bench_function("write_u64_large", |b| {
        b.iter(|| {
            write_u64(
                black_box(18446744073709551615),
                black_box(&mut buf),
                0,
            )
        })
...
fn benchmark_special_functions(c: &mut Criterion) {
    ...
    group.bench_function(
        "write_tag_and_current_timestamp",
        |b| {
            b.iter(|| {
                write_tag_and_current_timestamp(
                    black_box(&mut buf),
    ...
    group.bench_function("encode_base36_fixed13", |b| {
        b.iter(|| {
            encode_base36_fixed13(
                black_box(&mut buf),
                0,
                black_box(12345678901234567890),
            )
        })
    });
    ...
    group.bench_function(
        ...
        |b| {
            b.iter(|| {
                ...
                pos += write_tag_and_current_timestamp(
                    black_box(&mut buf),
                    pos,
                    b"52=",
                );
                pos += write_tag_and_ClOrdID(
                    black_box(&mut buf),
                    pos,
                    b"11=",
                );
                ...
                pos += write_tag_and_u64(
                    black_box(&mut buf),
                    pos,
                    b"38=",
                    1000,
                );
                pos += write_tag_and_f64(
                    black_box(&mut buf),
                    pos,
                    b"44=",
                    123.45,
                );
                pos += write_tag_and_bool(
                    black_box(&mut buf),
                    pos,
                    b"59=",
                    false,
...
fn benchmark_forge_buffer(c: &mut Criterion) {
    ...
    group.bench_function(
        "forge_out_buffer_creation",
        |b| b.iter(|| black_box(forge_out_buffer("FIX.4.4"))),
    );

    group.bench_function(
        "forge_out_buffer_with_immediate_write",
        |b| {
            b.iter(|| {
